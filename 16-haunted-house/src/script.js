...
const gravyGeometry = new THREE.BoxBufferGeometry(0.6, 0.8, 0.2)
...
for (let i = 0; i < 50; i++) {
    ...
    const grave = new THREE.Mesh(gravyGeometry, graveMaterial)
...
gui.add(doorLight, 'intensity').min(0).max(1).step(0.001).name('DL intensity')
gui.add(doorLight, 'distance').min(0).max(30).step(0.001).name('Dl distance')
...
const tick = () => {
    ...
    ghost2.position.y = Math.sin(ghost2Angle * 4) + Math.sin(elapsedTime * 2.4)
